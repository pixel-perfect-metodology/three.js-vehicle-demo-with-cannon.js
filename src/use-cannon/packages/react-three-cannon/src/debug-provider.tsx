import { propsToBody } from "use-cannon/packages/cannon-worker-api/src";
...
export function DebugProvider({
  ...
  const [{ bodies, bodyMap }] = useState<DebugInfo>({
    bodies: [],
    bodyMap: {},
  });
  const { refs } = usePhysicsContext();
  const [scene] = useState(() => new Scene());
  const cannonDebuggerRef = useRef(
    impl(scene, { bodies } as World, { color, scale })
  );
