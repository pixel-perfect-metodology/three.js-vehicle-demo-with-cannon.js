...
const isHingeConstraint = (c: unknown): c is HingeConstraint =>
  c instanceof HingeConstraint;
  ...
  switch (data.op) {
    ...
    case "removeBodies": {
      for (let i = 0; i < data.uuid.length; i++) {
        ...
        const key = Object.keys(state.subscriptions).find(
          (k) => state.subscriptions[k][0] === data.uuid[i]
        );
        if (key) {
    ...
    case "setPosition":
      state.bodies[data.uuid].position.set(
        data.props[0],
        data.props[1],
        data.props[2]
      );
      break;
    case "setQuaternion":
      state.bodies[data.uuid].quaternion.set(
        data.props[0],
        data.props[1],
        data.props[2],
        data.props[3]
      ...
      break;
    case "setRotation":
      state.bodies[data.uuid].quaternion.setFromEuler(
        data.props[0],
        data.props[1],
        data.props[2]
      );
      break;
    case "setVelocity":
      state.bodies[data.uuid].velocity.set(
        data.props[0],
        data.props[1],
        data.props[2]
      );
    ...
    case "setAngularVelocity":
      state.bodies[data.uuid].angularVelocity.set(
        data.props[0],
        data.props[1],
        data.props[2]
      );
    ...
    case "setLinearFactor":
      state.bodies[data.uuid].linearFactor.set(
        data.props[0],
        data.props[1],
        data.props[2]
      );
    ...
    case "setAngularFactor":
      state.bodies[data.uuid].angularFactor.set(
        data.props[0],
        data.props[1],
        data.props[2]
      );
    ...
    case "setMass":
      ...
      break;
    case "setMaterial":
      state.bodies[data.uuid].material = data.props
        ? createMaterial(data.props)
        : null;
      break;
    ...
    case "setFrictionGravity":
      state.world.frictionGravity = data.props
        ? new Vec3(...data.props)
        : undefined;
      break;
    case "setIsTrigger":
    ...
    case "setBroadphase":
      // @ts-expect-error
      state.world.broadphase = new (broadphases[`${data.props}Broadphase`] ||
        NaiveBroadphase)(state.world);
      break;
    case "setAxisIndex":
      if (state.world.broadphase instanceof SAPBroadphase) {
        state.world.broadphase.axisIndex =
          data.props === undefined || data.props === null ? 0 : data.props;
      }
      break;
    case "applyForce":
      state.bodies[data.uuid].applyForce(
        new Vec3(...data.props[0]),
        new Vec3(...data.props[1])
      );
      break;
    case "applyImpulse":
    ...
    case "applyLocalForce":
      state.bodies[data.uuid].applyLocalForce(
        new Vec3(...data.props[0]),
        new Vec3(...data.props[1])
      );
      break;
    case "applyLocalImpulse":
      state.bodies[data.uuid].applyLocalImpulse(
        new Vec3(...data.props[0]),
        new Vec3(...data.props[1])
      );
      break;
    ...
    case "removeConstraint":
      ...
      if (state.constraints[data.uuid]) {
        state.world.removeEventListener(
          "postStep",
          state.constraints[data.uuid]
        );
    ...
    case "removeRaycastVehicle": {
      state.world.removeEventListener(
        "preStep",
        state.vehicles[data.uuid].preStep
      );
      state.world.removeEventListener(
        "postStep",
        state.vehicles[data.uuid].postStep
      );
      state.vehicles[data.uuid].vehicle.world = null;
      delete state.vehicles[data.uuid];
      const key = Object.keys(state.subscriptions).find(
        (k) => state.subscriptions[k][0] === data.uuid
      );
      if (key) {
        delete state.subscriptions[key];
      }
