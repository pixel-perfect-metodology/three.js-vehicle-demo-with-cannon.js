...
import type { StructuredSerializeOptions } from "./worker/types";
...
type Operation<T extends OpName, P> = { op: T } & (P extends symbol
  ? {}
  : { props: P });
type WithUUID<T extends OpName, P = symbol> = Operation<T, P> & {
  uuid: string;
};
type WithUUIDs<T extends OpName, P = symbol> = Operation<T, P> & {
  uuid: string[];
};
...
type AtomicMessage<T extends AtomicName> = WithUUID<
  SetOpName<T>,
  AtomicProps[T]
>;
...
export type CannonMessageMap = {
  ...
  addSpring: WithUUID<
    "addSpring",
    [uuidA: string, uuidB: string, options: SpringOptns]
  >;
  applyForce: WithUUID<"applyForce", [force: Triplet, worldPoint: Triplet]>;
  applyImpulse: WithUUID<
    "applyImpulse",
    [impulse: Triplet, worldPoint: Triplet]
  >;
  applyLocalForce: WithUUID<
    "applyLocalForce",
    [force: Triplet, localPoint: Triplet]
  >;
  applyLocalImpulse: WithUUID<
    "applyLocalImpulse",
    [impulse: Triplet, localPoint: Triplet]
  >;
  ...
  setRaycastVehicleBrake: WithUUID<
...
export type CannonMessageBody<T extends OpName> = Omit<
  CannonMessageMap[T],
  "op"
>;
export type CannonMessageProps<T extends OpName> = CannonMessageMap[T] extends {
  props: unknown;
}
